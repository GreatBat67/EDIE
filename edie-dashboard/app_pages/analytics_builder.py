    ...
    if st.button("Correlate", type="primary", icon=":material/compare_arrows:"):
        ...
        if join_dim == "State":
            ...
            if not state_perspectives or not state_perspectives2:
                ...
                try:
                    corr_sql = f"WITH a AS ({sql_a}), b AS ({sql_b}) SELECT a.DIM AS STATE, a.VAL_A AS \"{d1}: {m1}\", b.VAL_B AS \"{d2}: {m2}\" FROM a JOIN b ON a.DIM = b.DIM ORDER BY a.VAL_A DESC"
                    corr_df = run_query(corr_sql)
                    st.scatter_chart(corr_df, x=f"{d1}: {m1}", y=f"{d2}: {m2}")
            ...
            if not month_perspectives or not month_perspectives2:
                ...
                try:
                    corr_sql = f"WITH a AS ({sql_a}), b AS ({sql_b}) SELECT a.DIM AS MONTH, a.VAL_A AS \"{d1}: {m1}\", b.VAL_B AS \"{d2}: {m2}\" FROM a JOIN b ON a.DIM = b.DIM ORDER BY a.DIM"
                    corr_df = run_query(corr_sql)
                    st.line_chart(corr_df, x="MONTH", y=[f"{d1}: {m1}", f"{d2}: {m2}"])
                    st.dataframe(corr_df, hide_index=True, use_container_width=True)
                except Exception as e:
                    st.error(f"Correlation error: {str(e)}")
