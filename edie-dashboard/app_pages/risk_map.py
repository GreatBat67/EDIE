...
<div class="edie-hero-banner">
    <div style="display: flex; justify-content: space-between; align-items: center;">
        <div>
            <h2 style="margin: 0; color: #FFFFFF; font-weight: 700;">US Geographic Risk Intelligence</h2>
            <p style="margin: 4px 0 0 0; color: #94A3B8; font-size: 0.95rem;">Interactive Map Overlaying Claims Density, NWS Weather Severity, and FEMA Exposure</p>
        </div>
        <div style="text-align: right; background: rgba(239, 68, 68, 0.15); border: 1px solid #EF4444; padding: 6px 14px; border-radius: 8px;">
            <span style="font-size: 0.8rem; color: #FCA5A5; font-weight: 600;">ACTIVE EXPOSURE</span>
            <div style="font-size: 1.1rem; font-weight: 700; color: #F8FAFC;">18 US Operating States</div>
        </div>
    </div>
</div>
...
# --- MAP VIEW (US-Focused with Color-Coded Risk Levels & Tooltips) ---
...
if not geo_risk.empty:
    ...
    color_map = {
        ...
        "HIGH": [249, 115, 22, 190],       # Orange
        "ELEVATED": [234, 179, 8, 170],    # Yellow
        "NORMAL": [34, 197, 94, 150]       # Green
    }
    ...
    for _, row in geo_risk.iterrows():
        ...
        if state in STATE_COORDS:
            ...
            map_data.append({
                ...
                "state": state,
                "risk": risk,
                "color": color_map.get(risk, [100, 100, 100, 150]),
                "claims": int(row["CLAIMS"]),
                "incurred": fmt_currency(row["TOTAL_INCURRED"]),
                "loss_ratio": f"{row['LOSS_RATIO']}%",
                "radius": max(45000, min(160000, int(row["CLAIMS"]) * 85)),
            })

    if map_data:
        map_df = pd.DataFrame(map_data)
        
        # PyDeck Scatterplot Layer locked to US Center
        layer = pdk.Layer(
            "ScatterplotLayer",
            data=map_df,
        ...
        st.pydeck_chart(
            pdk.Deck(
                ...
                tooltip={"html": "<b>State:</b> {state}<br/><b>Risk:</b> {risk}<br/><b>Claims:</b> {claims}<br/><b>Incurred:</b> {incurred}<br/><b>Loss Ratio:</b> {loss_ratio}", "style": {"color": "white", "backgroundColor": "#0F172A", "fontSize": "13px"}},
            )
        )
        
        # Map Legend
        lcol1, lcol2, lcol3, lcol4 = st.columns(4)
        lcol1.markdown("🔴 **Critical Risk** (Loss Ratio >70%)")
        lcol2.markdown("🟠 **High Risk** (Loss Ratio >50%)")
        lcol3.markdown("🟡 **Elevated** (Weather Alerts >1k)")
        lcol4.markdown("🟢 **Normal Risk**")
